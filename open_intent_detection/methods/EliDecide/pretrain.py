...
from utils.functions import save_model, restore_model, centroids_cal, euclidean_metric
from dataloaders.bert_loader import convert_examples_to_features


class PretrainManager:
    
    def __init__(self, args, data, model, centroids = None, delta = None,rotate_matrix = None, logger_name = 'Detection'):
        ...
        if args.pretrain or (not os.path.exists(args.model_output_dir)) or rotate_matrix is not None:
            ...
            if args.backbone == 'bert_disaware':
                ...
                #self.train_plain(args, data, centroids, delta, rotate_matrix, True)

            self.logger.info('Pre-training finished...')
                
        else:
            self.model = restore_model(self.model, args.model_output_dir).to(self.device)
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(self.train_dataloader, desc="Iteration")):
                ...
                with torch.set_grad_enabled(True):
                    loss = self.model(input_ids, segment_ids, input_mask, label_ids, mode = "train", use_con=True)
            ...
            tot_loss = tr_loss / nb_tr_steps
            
            y_true, y_pred = self.get_outputs(args, data, mode = 'eval')
            eval_score = round(accuracy_score(y_true, y_pred) * 100, 2)# - loss

            eval_results = {
                'train_loss': tot_loss,
                # 'train_disloss': dis_loss.item(),
    ...
    def get_outputs(self, args, data, mode = 'eval', get_feats = False):
        ...
        # if args.backbone == 'bert_con'or args.backbone == 'bert_con_eus':
        centroids = centroids_cal(self.model, args, data, self.train_dataloader, self.device, False, False)
        ...
        for batch in tqdm(dataloader, desc="Iteration"):
            ...
            with torch.set_grad_enabled(False):
                ...
                # elif args.backbone == 'bert_con' or args.backbone == 'bert_con_eus':
